...
shared_ptr<Node> invert_tree(shared_ptr<Node> root){
    if(root == nullptr){
        return nullptr;
    }
    shared_ptr<Node> right = invert_tree(root->left);
    shared_ptr<Node> left = invert_tree(root->right);
    root->left = left;
    root->right = right;
    return root;
}
...
void  invert_binary_tree(BinaryTree& original_binary_tree){
    invert_tree(original_binary_tree.root);
}
